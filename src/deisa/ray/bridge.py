...
class Bridge:
    ...
    def __init__(
        ...
        *args, **kwargs
        ...
        node_actor_options: Dict[str, Any] = get_node_actor_options(
            name, namespace)
    ...
    def validate_arrays_meta(
        ...
        for array_name, meta in arrays_metadata.items():
            ...
            if not isinstance(array_name, str):
                raise TypeError(
                    f"arrays_metadata keys must be str, got {
                        type(array_name).__name__}"
                )
            ...
            if missing:
                raise ValueError(
                    f"arrays_metadata['{array_name}'] is missing required keys: {
                        missing}"
                )
    ...
    def _validate_single_array_metadata(
        ...
        if not (isinstance(nb_chunks_of_node, int) and nb_chunks_of_node > 0):
            raise TypeError(
                f"arrays_metadata['{
                    name}']['nb_chunks_of_node'] must be a positive int, "
    ...
    def validate_system_meta(self, system_meta: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if not isinstance(system_meta, Mapping):
            raise TypeError(
                f"system_metadata must be a mapping, got {
                    type(system_meta).__name__}"
            )
    ...
    def create_node_actor(
        ...
        #  encountering a `break`.
            ...
            raise RuntimeError(
                f"Failed to create/ready scheduling actor for node {
                    self.node_id}"
            ) from last_err
    ...
    def get(
        ...
        **kwargs: Any,
        ...
        raise NotImplementedError(
            "Retrieving chunked arrays via Bridge.get is not implemented yet.")
