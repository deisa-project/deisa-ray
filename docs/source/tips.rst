...
Determining a rendezvous IP address and port
--------------------------------------------

When launching a simulation across multiple ranks, it is often convenient to
have rank 0 choose the rendezvous address and port for a custom TCP-based
communicator, then share them with the other ranks through a small JSON file.
...
pick a free port, and writes both values to ``comm_rdzv.json``. All other
