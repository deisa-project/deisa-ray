...
A preprocessing callback is a function that is applied on each chunk of data.
The function is executed locally, on the machine where the data is produced as
...
It is possible to define a callback that will be executed a bit before the data
is ready. The return value of this callback will be passed as an argument to
the actual simulation callback.
...
In most situations, this is negligible, but it can start to matter with
simulations running on hundreds of nodes, with task graphs composed of
thousands of tasks.
